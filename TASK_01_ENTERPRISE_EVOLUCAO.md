...
- [x] E4-T1: Nova área “Administração” (tenant/usuários/permissões).
- [x] E4-T2: Guardas de navegação por permissão.
- [x] E4-T3: Banner/estado para limites do plano e tenant suspenso.
- [x] E4-T4: Feature flags por plano com fallback amigável.

## EPIC E5 — Observabilidade e compliance
- [x] E5-T1: Correlation ID no gateway + logs estruturados por request.
- [x] E5-T2: Auditoria de ações críticas com rastreabilidade.
- [x] E5-T3: Dashboard operacional (erros, latência, quotas).
- [x] E5-T4: Runbook de incidentes e recuperação.
...
### Atualização Sprint 4 (E4) — executado
- Console Admin Enterprise adicionado no frontend (tenant, usuários, estado da assinatura).
- UX de permissões aplicada com guardas de navegação por página e bloqueio com feedback visual.
- Feature flags por plano (`Pro`/`Enterprise`) aplicadas no menu e acesso ao console admin.
- Banner de estado enterprise implementado para sinalizar restrições de plano/permissão e tenant inativo.


### Atualização Sprint 5 (E5) — executado
- Correlation ID (`request_id`) adicionado no gateway, logs e auditoria.
- Auditoria reforçada com rastreabilidade por request/tenant e códigos de erro.
- Endpoint `observabilidade.status` adicionado para visão operacional por tenant.
- Hardening final aplicado: bloqueio de operações quando tenant está inativo (exceto `billing.assinatura.status`).
- Runbook operacional adicionado em `TASK_02_RUNBOOK_OBSERVABILIDADE_COMPLIANCE.md`.


### Atualização Sprint 6 (Piloto controlado) — executado
- Rollout controlado de recursos enterprise por tenant com capacidade limitada (1-2 tenants configurável por propriedade).
- Rotas enterprise críticas bloqueadas para tenants fora do piloto (`enterprise_pilot_restrito`) e para plano não-Enterprise (`enterprise_plan_required`).
- Rotas administrativas do piloto adicionadas: `admin.piloto.status`, `admin.piloto.inscrever`, `admin.piloto.remover`.
- Contratos da API ampliados para validar `admin.piloto.status` em `api_version=v2`.
