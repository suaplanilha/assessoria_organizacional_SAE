...
## 10) Andamento de execução

### P1 (Fase 1 + Fase 2)
- Implementado no frontend: dedupe/caching/invalidação de cache por domínio + persistência de alteração de plano SaaS via billing.

### P2 (Fase 3)
- Hardening inicial aplicado em front/back com foco em:
  - validações de payload (`validation_error`) em clientes, tarefas, diagnósticos, financeiro e relatórios;
  - códigos explícitos para `forbidden`, `cliente_not_found`, `registro_not_found`;
  - mensagens de erro no frontend distinguindo erro técnico e erro de negócio em mais módulos (incluindo portal e relatórios).

### P3 (Fase 4)
- Performance/caching iniciado com:
  - paginação + cache server-side em `diagnosticos.listar` e `financeiro.listar` (incluindo `no_cache/force_fresh`);
  - roteamento API atualizado para repassar parâmetros de paginação dessas rotas;
  - otimização de `getDadosPortalCliente` para evitar leitura full-sheet de clientes;
  - frontend ajustado para enviar paginação explícita e `forceFresh` em mutações para evitar dado obsoleto.

### P4 (Fase 5)
- QA/regressão contínuo iniciado com:
